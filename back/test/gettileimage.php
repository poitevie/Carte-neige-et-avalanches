...
if (
    ...
    if (is_int($y) && is_int($x) && $y >= 0 && $y <= 12 && $x >= 0 && $x <= 12) {
        ...
        if (!$fp = fopen($filespath . $filenumber . $fileext, "rb"))
            ...
            $image = imagecreatetruecolor(277, 277);
            for ($j = 0; $j < $img_size; $j++) {
                for ($i = 0; $i < $img_size; $i++) {
                    ...
                    $blue = imagecolorallocate($image, 255*(sin($alt/300)+1)/2, 255*(sin($alt/300 + 2*3.14/3)+1)/2, 255*(sin($alt/300 - 2*3.14/3)+1)/2);
                    imagesetpixel($image, $i, $j, $blue);
                }
            }
